...
Приглашение прислать тексты и предложение записи появляются, только когда задача человека — сам текст: написать, переписать или проверить его. Если скилл работает как правило внутри другой работы (код, дизайн, настройка, ответ на вопрос), файлы голоса читаются и применяются, но ни приглашения, ни предложения записи в ответе нет.
