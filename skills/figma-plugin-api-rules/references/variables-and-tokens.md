...
_Not always: in some files the same call resolves such IDs to names (`harvest-variable-ids-from-bound-nodes-when-local-collection-is-empty` below), and what separates the two cases is unknown. Handle both outcomes: use the variable when the call resolves, copy the paint whole when it returns `null`._
**Principle:** `figma.variables.getVariableByIdAsync` can silently return `null` (no exception) for a remote/library variable whose ID was taken from another node's `boundVariables` (format `<hash>/<num>:<num>`, not `VariableID:X:Y`), and `setBoundVariableForPaint(paint, 'color', null)` silently erases the binding — instead of resolving, copy `color` + `boundVariables` (`{color: {type: 'VARIABLE_ALIAS', id}}`) whole from the reference paint into the new paint object.
...
```js
// ❌ WRONG — when getVariableByIdAsync returns null for a remote variable, the binding is silently lost
...
**Principle:** The number of modes per variable collection is capped by plan — 10 on Professional, 20 on Organization, no fixed cap on Enterprise (September 2026; current numbers in Figma's "Plans and features" help article) — and `collection.addMode(name)` past the cap throws `"Limited to N modes only"`; this is an architectural constraint of the whole DS strategy; there is no technical workaround.
...
_In some files the same call returns `null` for these IDs — the fallback is in `getvariablebyidasync-null-remote-variable` above._
...
**Pattern:** remember the donor paint's `opacity` BEFORE binding; bind in one `use_figma` call, then restore the opacity on a plain copy in a SEPARATE next call, on top of the already-bound fills. A plain copy within the same call does not reliably apply, even with a double `JSON.parse(JSON.stringify(...))` round trip: no error, `fills` is assigned, the readback shows `opacity: 1`. Plan two calls by default; if the first happened to keep the opacity, the second becomes a harmless no-op.
```js
// call 1 — binding only
const srcOpacity = bar.fills[0].opacity;               // remember BEFORE
...
flat.opacity = 0.7;                                    // the value remembered in call 1
bar.fills = [flat];
...
**Principle:** "Rebind the glyph from `text/<role>` to `icon/<role>`" is a semantic edit but NOT necessarily a visual one: in a semantic collection both variables often alias the same primitive in EVERY mode. Before declaring such a rebind "visible in N themes" and requiring the user's decision — unwrap the alias down to the primitive in every mode of both variables and compare. The number of modes is identical for variables of one collection by construction; only the alias target can diverge.
**Symptom:** a claim that the rebind "is visible in N themes, because one variable is defined in more modes than the other" — while a measurement gives one collection, the same modes for both and identical primitives.
**Pattern:**
```js
...
**Symptom:** neighbouring roles of one task, fixed the two different ways, give diverging results. A primitive used by nothing but the target roles, edited in place, also fixes every other brand/mode that aliases it — "one fix closes both brands" works literally. A primitive that is also needed untouched elsewhere (e.g. in the dark-mode border or a fill) forces a new primitive + a re-point of ONLY one mode's value; another brand that aliased the old primitive keeps the old value until a SEPARATE explicit re-point aimed at its own value — nothing "arrives" by itself.
...
**Symptom:** a monotonicity check script (written via HSL-L as a cheaper/more intuitive proxy for real brightness) returns `monotonic: true`, but `fill/bg/tertiary` is visually/really lighter than `fill/bg/secondary` — easy to fall into when a new ad hoc interpolation script takes HSL-L "for speed" instead of WCAG luminance.
